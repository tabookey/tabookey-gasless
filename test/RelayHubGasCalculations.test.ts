...
import { calculateTransactionMaxPossibleGas, decodeRevertReason, getEip712Signature } from '../src/common/Utils'
      ...
      it(`test data cost with datasize ${bufferExtra}`, async function () {
        ...
        async function checkAcceptanceBudget (gas: number): Promise<boolean> {
          ...
          try {
            ...
            // console.log('acceptanceBudget=', gas, 'ret=', txrej != null, 'innerGasUsed=', txrej?.args?.innerGasUsed?.toString(), Buffer.from(txrej?.args?.reason.slice(2) ?? '', 'hex').toString(), 'relayed:',
        ...
        const budget = await bsearch(0, 200000, checkAcceptanceBudget)
